...
void logp::open(int bot_id) {
    if (has_opened) {
        logp::log("Error: log: tried to open(" + std::to_string(bot_id) + ") but we have already opened before.");
        exit(1);
    }
...
void logp::log(const std::string& message) {
    if (has_opened) {
        log_file << message << std::endl;
    } else {
        if (!has_atexit) {
